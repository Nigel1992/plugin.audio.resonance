...
class SimpleCache(object):
    ...
    def set(self, endpoint, data, checksum="", expiration=datetime.timedelta(days=30), json_data=False):
        ...
        # Compatibility fix:
        # Some addons pass expiration as seconds (int)
        # while old simplecache expects datetime.timedelta.
        if isinstance(expiration, int):
            expiration = datetime.timedelta(
                seconds=expiration
            )

        expires = self._get_timestamp(
    ...
    def delete(self, endpoint):
        '''delete one cache object from memory and sqlite'''
        endpoint = str(endpoint or "")
        if not endpoint:
            return
        self._win.clearProperty(self._mem_key(endpoint))
        self._execute_sql("DELETE FROM simplecache WHERE id = ?", (endpoint,))

    def delete_prefix(self, prefix):
        '''delete all cache objects whose ids begin with prefix'''
        prefix = str(prefix or "")
        if not prefix:
            return
        rows = self._execute_sql(
            "SELECT id FROM simplecache WHERE id LIKE ?", (prefix + "%",)
        )
        for row in rows.fetchall() if rows else []:
            self._win.clearProperty(self._mem_key(row[0]))
        self._execute_sql("DELETE FROM simplecache WHERE id LIKE ?", (prefix + "%",))

    def check_cleanup(self):
        '''check if cleanup is needed - public method, may be called by calling addon'''
        cur_time = datetime.datetime.now()
        lastexecuted = self._win.getProperty(self._mem_key("simplecache.clean.lastexecuted"))
        if not lastexecuted:
            self._win.setProperty(self._mem_key("simplecache.clean.lastexecuted"), repr(cur_time))
        elif (eval(lastexecuted) + self._auto_clean_interval) < cur_time:
            # cleanup needed...
            self._do_cleanup()

    def _mem_key(self, endpoint):
        # Kodi Window(10000) properties are global across addons. Namespace every
        # memory key so plugin.audio.resonance can never consume plugin.audio.spotify cache.
        return "%s.%s" % (self.addon_id, endpoint)
    ...
    def _get_mem_cache(self, endpoint, checksum, cur_time, json_data):
        ...
        '''
        result = None
        cachedata = self._win.getProperty(self._mem_key(endpoint))
    ...
    def _set_mem_cache(self, endpoint, checksum, expires, data, json_data):
        ...
        self._win.setProperty(self._mem_key(endpoint), cachedata_str)
    ...
    def _do_cleanup(self):
        ...
        if self._win.getProperty(self._mem_key("simplecachecleanbusy")):
            return
        self._win.setProperty(self._mem_key("simplecachecleanbusy"), "busy")
        ...
        for cache_data in self._execute_sql(query).fetchall():
            ...
            self._win.clearProperty(self._mem_key(cache_id))
        ...
        self._win.setProperty(self._mem_key("simplecache.clean.lastexecuted"), repr(cur_time))
        self._win.clearProperty(self._mem_key("simplecachecleanbusy"))
    ...
    def _get_database(self):
        ...
        try:
            ...
            # Close the first connection before deleting its file. Windows
            # otherwise rejects the delete because sqlite still owns a handle.
