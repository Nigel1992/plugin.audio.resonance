...
class Spotify:
    ...
    def _internal_call(self, method, url, payload, params):
        ...
        try:
            ...
            # Spotify's current playlist migration can return 403/404 while
            # callers deliberately probe the alternate /items or deprecated
            # /tracks read endpoint.  Keep those expected capability results
            # visible without presenting them as an unhandled Kodi error.
            log_http_error = (
                logger.warning
    ...
    def search(self, q, limit=10, offset=0, type="track", market=None, include_external=None):
        """ searches for an item

            Parameters:
                - q - the search query (see how to write a query in the
                      official documentation https://developer.spotify.com/documentation/web-api/reference/search/)  # noqa
                - limit - the number of items to return (min = 1, default = 5, max = 10). The limit is applied
                ...
                - market - An ISO 3166-1 alpha-2 country code or the string
                           from_token.
                - include_external - use "audio" when the client can play
                                     externally hosted podcast audio.
        """
        return self._get(
            "search", q=q, limit=min(10, max(1, int(limit or 5))), offset=offset,
            type=type, market=market, include_external=include_external
        )
    ...
    def current_user_saved_audiobooks(self, limit=20, offset=0):
        """Get audiobooks saved in the current user's Spotify library."""
        return self._get(
            "me/audiobooks", limit=max(1, min(int(limit), 50)),
            offset=max(0, int(offset))
        )

    def current_user_library_contains(self, uris):
        """Check URI-based Your Library membership (Spotify 2026 API)."""
        return self._get("me/library/contains", uris=",".join(uris or []))

    def current_user_library_add(self, uris):
        """Save Spotify URIs with the generic Your Library endpoint."""
        return self._put("me/library", uris=",".join(uris or []))

    def current_user_library_remove(self, uris):
        """Remove Spotify URIs with the generic Your Library endpoint."""
        return self._delete("me/library", uris=",".join(uris or []))
